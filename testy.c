...
void testy() {
...
#ifdef TEST1
    {
        ...
        errno = 1;
        bool uninitialed_input =
            nand_input(g0, 6) != NULL || nand_input(g2, 0) != NULL;
        bool errno_not_set_to_zero =
            errno != 0; // nand_input() ma ustawiać errno na zero w przypadku
                        // gdy zwraca pustą wartość
...
#ifdef TEST5
    {
        ...
        // dla nand_output(), wynik ma być niezdefiniowany dla niepoprawnego
        // inputu wedle specyfikacji
        // assert(nand_output(NULL, 0) == NULL && errno == EINVAL);
        // errno = 0;
        // assert(nand_output(g1, 2) == NULL && errno == EINVAL);
        // errno = 0;
        // assert(nand_output(g1, -1) == NULL && errno == EINVAL); // problem z
        // porównaniem (ssize_t)(-1) < (size_t)(2) == 0 ? errno = 0;
    ...
    printf("Uwaga! Testerka nie sprawdza, czy program poprawnie reaguje na "
           "problemy z alokacja pamieci.\nUpewnij sie, ze spojnosc programu "
           "jest zachowana nawet gdy malloc zwroci NULL\n");
...
#ifdef TEST6
    {
        ...
        // Kwestia sporna, no bo co oznacza ścieżka krytyczna dla żadnych
        // bramek? Ponadto przyznanie, że nand_evaluate(&g, &b, 0) coś zwraca,
        // pociąga za sobą przyznanie, że nand_evaluate(NULL, NULL, 0) również.
